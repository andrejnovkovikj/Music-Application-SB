...
const ListSong = ({ isAuthenticated, currentUser }) => {
    ...
    const fetchLikedSongs = async () => {
        ...
        try {
            ...
            setLikedSongs(response.data.map(song => song.id)); // Store only song IDs
        } catch (error) {
            console.error("Error fetching liked songs:", error);
        }
    };

    // Fetch Songs and Check Admin Role
    useEffect(() => {
        ...
        const checkAdminRole = async () => {
            try {
                ...
                setIsAdmin(response.data); // API returns `true` for admin, `false` otherwise
            } catch (error) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this song?")) {
            try {
                ...
                setSongs(prevSongs => prevSongs.filter(song => song.id !== id)); // Immediately update state
